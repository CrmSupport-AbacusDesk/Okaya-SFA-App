...
app.controller('loginCtrl', function ($scope, $rootScope, searchSelect, $ionicModal, $state, myRequestDBService, myAllSharedService, $ionicLoading, $cordovaSQLite, $ionicPopup, $timeout, $ionicActionSheet, $cordovaImagePicker, Camera, $cordovaFileTransfer, $cordovaGeolocation, $cordovaToast, $location, $window, $ionicPlatform, $ionicHistory, $ionicModal,$stateParams, $ionicScrollDelegate, $ionicPopover) {
      ...
      $scope.openCataloguePdf = function (pdfName, title)
      {
            ...
            $scope.getProductCategory = function()
            {
                  ...
                  myRequestDBService.orpPostServiceRequest("/ORP_Controller/GET_COMPLAINT_CATEGORY","").then(function(response)
                  {
                        console.log(response);
            ...
            $scope.getProductSubCategory = function(category_name)
            {
                  $scope.enableInput=false;
                  if(category_name == 'Other'){
                        $scope.enableInput=true;
                        $scope.suportData.sub_category = '';
                        return;
                  }
                  ...
                  myRequestDBService.orpPostServiceRequest("/ORP_Controller/GET_COMPLAINT_CATEGORY_SUBJECT",{'category_name': category_name}).then(function(response)
                  {
                        console.log(response);
                        ...
                        myRequestDBService.orpPostServiceRequest("/ORP_Controller/saveSupportData",{data:$scope.suportData}).then(function(response)
                        {
                              ...
                              if($rootScope.documentFiles.length)
                              {
                                    for (let i = 0; i < $rootScope.documentFiles.length; i++) {
                                          ...
                                          $http.post(orpServerURL + "/ORP_Controller/support_document_data?support_id=" + response.inserted_id + "&dr_code=" +$scope.loginData['drCode'], fd, {
            ...
            if($location.path() == '/tab/orp_support')
            {
